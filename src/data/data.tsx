export const data = {
  items: [
    {
      id: 1,
      name: undefined,
      title: "Découvrez Tesla",
      bookTestDrive: "Réserver un essai aujourd'hui",
      bookTestDriveSlug: "discoverTesla",
      className: "discover-tesla",
      backgroundDesktop: "/media/Homepage-Test_Drive.mp4",
      backgroundMobile: "/media/Homepage-Test_Drive-Mobile.mp4",
      buttonDarkText: undefined,
      buttonLightText: undefined,
      buttonTransparent: "Essais",
    },
    {
      id: 2,
      name: "Model Y",
      title: "Model Y",
      bookTestDrive: "Réserver un essai",
      bookTestDriveSlug: "testdrive",
      slug: "modely",
      className: "modely",
      backgroundDesktop: "/media/modely.avif",
      backgroundMobile: "/media/modely.avif",
      buttonDarkText: "Véhicules disponibles",
      buttonLightText: "Configuration personalisée",
    },
    {
      id: 3,
      name: "Model 3",
      title: "Model 3",
      bookTestDrive: "Réserver un essai",
      bookTestDriveSlug: "testdrive",
      slug: "model3",
      className: "model3",
      backgroundDesktop: "/media/model3.avif",
      backgroundMobile: "/media/model3.avif",
      buttonDarkText: "Véhicules disponibles",
      buttonLightText: "Configuration personalisée",
    },
    {
      id: 4,
      name: "Model S",
      title: "Model S",
      bookTestDrive: "Réserver un essai",
      bookTestDriveSlug: "testdrive",
      slug: "models",
      className: "models",
      backgroundDesktop: "/media/models.avif",
      backgroundMobile: "/media/models.avif",
      buttonDarkText: "Configuration personalisée",
      buttonLightText: "En savoir plus",
    },
    {
      id: 5,
      name: "Model X",
      title: "Model X",
      bookTestDrive: undefined,
      bookTestDriveSlug: "",
      slug: "modelx",
      className: "modelx",
      backgroundDesktop: "/media/modelx.avif",
      backgroundMobile: "/media/modelx.avif",
      buttonDarkText: "Configuration personalisée",
      buttonLightText: "En savoir plus",
    },
    {
      id: 6,
      name: "Powerwall",
      title: "Systèmes d'énergie solaire et Powerwalls",
      bookTestDrive: "De l’énergie pour tous vos besoins",
      bookTestDriveSlug: "",
      slug: "powerwall",
      className: "powerwall",
      backgroundDesktop: "/media/powerwall.avif",
      backgroundMobile: "/media/powerwall.avif",
      buttonDarkText: "En savoir plus",
      buttonLightText: undefined,
    },
    {
      id: 7,
      name: "Recharge",
      title: "Accessoires",
      bookTestDrive: undefined,
      bookTestDriveSlug: "",
      slug: "recharge",
      className: "recharge",
      backgroundDesktop: "/media/charger.avif",
      backgroundMobile: "/media/charger.avif",
      buttonDarkText: "Commander maintenant",
      buttonLightText: undefined,
    },
  ],
  assistance: {
    name: "Assistance",
    slug: "assistance",
  },
  shop: {
    name: "Shop",
    slug: "shop",
  },
  account: {
    name: "Compte",
    slug: "compte",
  },
};
